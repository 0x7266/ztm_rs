...
fn authorize(
    ...
    let user = database.find_employee(employee_name)?;
    let keycard = database.get_keycard(&user)?;
...
fn main() {
    ...
    // John is trying to access the Warehouse, which requires access level 500.
    // John is not an employee.
    let john_authorized = authorize("John", ProtectedLocation::Warehouse);

    println!("{anita_authorized:?}");
    println!("{brody_authorized:?}");
    println!("{catherine_authorized:?}");
    println!("{john_authorized:?}");
}
